...
public class GridLayoutTests
{
    ...
    public void Layout_fields_in_a_two_column_grid()
    {
        ...
        grid.Children.Count.Should().Be(2 * fields.Length);
    ...
    public void Skip_rows_if_label_and_value_both_empty()
    {
        ...
        grid.Children.Count.Should().Be(2);
    ...
    public void Recurse_on_group_fields()
    {
        ...
        grid1.Children.Count.Should().Be(fields1.Length * 2); // because we have label and value
        foreach (var field in fields1)
        {
            grid1.Children.Contains(field.LabelControl).Should().BeTrue();
            grid1.Children.Contains(field.ValueControl).Should().BeTrue();
        }

        var grid2 = (Grid)content2.Content;
        grid2.Children.Count.Should().Be(fields2.Length * 2); // because we have label and value
        foreach (var t in fields2)
        {
            grid2.Children.Contains(t.LabelControl).Should().BeTrue();
            grid2.Children.Contains(t.ValueControl).Should().BeTrue();
        }
    }
