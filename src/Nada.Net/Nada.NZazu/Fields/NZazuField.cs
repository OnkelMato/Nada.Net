...
public abstract class NZazuField<T> : NZazuField, INZazuWpfField<T>
{
    ...
    public override ValueCheckResult Validate()
    {
        ...
        if (bindingExpression != null && bindingExpression.HasError)
            return new ValueCheckResult(new Exception(
                $"{Definition.Prompt}: {bindingExpression.ValidationError?.ErrorContent as string ?? "UI has errors. Value could not be converted"}"));
